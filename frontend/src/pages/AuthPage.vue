  ...
  <q-page class="auth-page">
    ...
    <main class="auth-panel">
      <section class="auth-shell">
        ...
        <div v-if="isProfessional && registerMode" class="registration-method" role="tablist" aria-label="Forma de cadastro profissional">
          <button type="button" role="tab" :aria-selected="professionalRegistrationMethod === 'google'" :class="{ active: professionalRegistrationMethod === 'google' }" @click="selectProfessionalRegistrationMethod('google')">
            <q-icon name="account_circle" /><span><b>Continuar com Google</b><small>Sem criar uma nova senha</small></span>
          </button>
          <button type="button" role="tab" :aria-selected="professionalRegistrationMethod === 'password'" :class="{ active: professionalRegistrationMethod === 'password' }" @click="selectProfessionalRegistrationMethod('password')">
            <q-icon name="mail_outline" /><span><b>Usar e-mail e senha</b><small>Cadastro tradicional</small></span>
          </button>
        </div>

        <q-banner v-if="googleProfessionalRegistration" rounded class="google-registration-note">
        ...
        <q-form class="auth-fields" @submit="submit">
          <div v-if="registerMode" :class="['register-grid', { 'register-grid--two': isProfessional && !googleProfessionalRegistration }]">
            <q-input v-if="!googleProfessionalRegistration" v-model.trim="form.name" outlined rounded label="Nome completo" autocomplete="name" :rules="[required('Informe seu nome.')]" lazy-rules>
          ...
          <q-input
            v-if="!googleProfessionalRegistration"
          ...
          <div v-if="!registerMode" class="login-meta">
            <span><q-icon name="shield" /> Dados deste acesso protegidos por até 120 dias</span>
            <router-link :to="forgotPasswordRoute">Esqueci minha senha</router-link>
          </div>
        ...
        <template v-if="googleEligible">
          <div class="auth-divider"><span>{{ googleProfessionalRegistration ? 'finalize com' : 'ou continue com' }}</span></div>
          <div class="google-profile-context">
            <q-icon :name="isProfessional ? 'storefront' : 'person_outline'" />
            <span>{{ googleContext }}</span>
          </div>
        ...
        <div v-else class="professional-google-hint"><q-icon name="info_outline" /> Selecione “Continuar com Google” para criar o perfil sem senha.</div>
...
const googleContext = computed(() => googleProfessionalRegistration.value
  ...
  : `O Google será usado para entrar como ${isProfessional.value ? 'profissional' : 'cliente'}.`)
...
const forgotPasswordRoute = computed(() => ({
  path: '/esqueci-senha',
  query: { perfil: isProfessional.value ? 'profissional' : 'cliente' }
}))
...
function selectProfessionalRegistrationMethod (value) {
  professionalRegistrationMethod.value = value
...
async function handleGoogle (response) {
  if (googleProfessionalRegistration.value) {
    ...
    if (phoneRule(form.phone) !== true) {
      $q.notify({ type: 'negative', message: 'Informe um telefone profissional válido com DDD antes de continuar com o Google.' })
      return
    }
    if (slugRule(form.slug) !== true) {
      $q.notify({ type: 'negative', message: 'A URL personalizada deve usar apenas letras minúsculas, números e hífens.' })
      return
    }
  }
  try {
