<template>
  <q-page class="dashboard-page">
    <div class="page-intro"><div><span class="page-overline">QUINTA-FEIRA, 13 DE AGOSTO</span><h1>Bom dia, {{ firstName }}.</h1><p>Aqui está o ritmo da sua barbearia hoje.</p></div><q-btn to="/barber/calendario" rounded unelevated color="dark" no-caps label="Novo agendamento" icon="add" /></div>
...
const auth = useAuthStore(); const stats = ref({}); const appointments = ref([]); const $q = useQuasar(); const router = useRouter()
...
async function loadDashboard () { const { data } = await api.get('/barber/dashboard'); stats.value = data.stats; appointments.value = data.nextAppointments }
